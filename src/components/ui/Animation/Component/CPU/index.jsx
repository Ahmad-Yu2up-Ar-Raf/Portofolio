...
import FadeContent from "@/components/ui/Animation/Content";
import styles from './style.module.css'
const Connecttors = (props) => {
  const { 
    length, 
    variants = "-top-[10px] gap-[18px]", 
    boxShadow = "0 2px 2px 1px rgba(0, 0, 0, .02), inset 0 -1px 1px 1px rgba(0, 0, 0, .1), inset 0 -1px 1px #dddddd",
    linear = "linear-gradient(180deg, #787878, #6b6b6b 33.33%, #8b8b8b 66.67%, #303030)",
    variant = "w-[6px] h-[10px]"
  } = props;
  
  return (
    <div className={`flex ${variants} absolute`}>
      {Array.from({ length: parseInt(length) }, (_, i) => (
        <span
          key={i}
          style={{
            background: linear,
  ...
  return (
    ...
    >
      ...
      <Connecttors
        ...
        linear="linear-gradient(1turn, #6f6f6f, #878787 33.33%, #8b8b8b 66.67%, #4d4d4d)"
        length="2"
        variants="flex-col -right-[10px] gap-[14px]"
        variant="w-[10px] h-[6px]"
      />
    </FadeContent>
  );
