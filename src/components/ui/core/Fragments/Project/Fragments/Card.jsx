...
export const Card = ({
    ...
    return (
      ...
      key={i}
      threshold={0.2}>
  <div className="project-card flex w-full flex-row font-oufit">
...
className={`inline rounded-sm border-white/[0.34] px-1.5 py-[3px] text-xs fill-${color}-600 text-${color}-400 bg-${color}-600/20 lg:bg-black`}>{stack}</li>
  ...
  <footer className="flex w-full gap-4 ">
    ...
    className={` ${ !Web ? 'pointer-events-none text-white/50' : ''    }  relative flex h-fit w-1/2 items-center justify-center gap-2 rounded-xl bg-neutral-900 px-5 py-2 shadow-[inset_0_2px_10px_#ffffff1f] after:inset-0 after:bg-black/50 `}>
